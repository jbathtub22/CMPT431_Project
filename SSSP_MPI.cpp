...
void SSSP(Graph &g, int source_vertex) 
{
    ...
    //int* count_arr;
    //count_arr = (int*)malloc(world_size * sizeof(int));
    ...
    //displc_arr = (int*)malloc(world_size * sizeof(int));
    ...
    //bool* marked;
    //marked = (bool*)malloc(local_n * sizeof(bool));
    //int* local_dist; 
    //local_dist = (int*)malloc(local_n * sizeof(int));
    //int* local_pred;
    //local_pred = (int*)malloc(local_n * sizeof(int));
    for (int i = 0; i < local_n; i++) {
        marked[i] = false;
        local_dist[i] = Inf;
    ...
    */
    ...
    //int* local_min_pair;
    //local_min_pair = (int*)malloc(2 * sizeof(int));
    //int* global_min_pair;
    //global_min_pair = (int*)malloc(2 * sizeof(int));
    //int* other_min_pair = malloc(2 * sizeof(int));
    ...
    for (int step = 0; step < total_n-1; step++) {
        ...
        /*  
        if (world_rank > 0) {
            MPI_Send(local_min_pair, 2, MPI_INT, 0, world_rank, MPI_COMM_WORLD);
        } else {
            global_min_pair[0] = local_min_pair[0];
            global_min_pair[1] = local_min_pair[1];

            for (int i = 1; i < world_size; i++) {
                MPI_Recv(other_min_pair, 2, MPI_INT, i, i, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        MPI_Bcast(global_min_pair, 2, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    if (world_rank == 0) {
        ...
        //global_dist = (int*)malloc(total_n * sizeof(int));
        //global_pred = (int*)malloc(total_n * sizeof(int));
    }
